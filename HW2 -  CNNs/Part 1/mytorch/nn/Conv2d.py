...
class Conv2d_stride1():
    ...
    def forward(self, A):
        ...
        for i in range(output_height):
            for j in range(output_width):
                Z[:, :, i, j] = np.tensordot(self.A[:, :, i: i+self.kernel_size, j: j + self.kernel_size] , self.W, axes = ([1, 2, 3], [1, 2, 3]))
    ...
    def backward(self, dLdZ):
        ...
        self.dLdb = np.sum(dLdZ, axis = (0,2, 3)) # i htink thi sis good
        ...
        dLdZ_padded = np.pad(dLdZ, ((0, 0), (0, 0), (pad_size, pad_size), (pad_size, pad_size)), mode='constant')
