...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            ...
            current_input_channels = num_channels[i]

            current_input_width = (current_input_width - kernel_sizes[i]) // strides[i] + 1

        self.convolutional_layers = self.convolutional_layers  

        # Flatten layer
        self.flatten = Flatten()

        # linear layer
        self.linear_layer = Linear( in_features=current_input_width * num_channels[-1],out_features=num_linear_neurons )
    ...
    def forward(self, A):
        ...
        flat = self.flatten.forward(A)
        # Save output (necessary for error and loss)
        self.Z = self.linear_layer.forward(flat)
